...
const BASE_SHARE_TEXT_LOSE = "I play Will vs Chris! Try it yourself";
...
export const Again = ({ onClick, score }) => {
  const encodedValue = React.useMemo(() => {
    ...
    return encodeURIComponent(BASE_SHARE_TEXT_LOSE);
  }, [score]);
  ...
  return (
    <div className="tryAgain">
      {score ? <div>Your Score: {score}</div> : null}
      {sharing}
      <iframe
        title="fbShare"
      ...
      >
        ...
        <span className="twitter-label">
          {score ? "Tweet Your Score" : "Share on Twitter"}
        </span>
      </a>
